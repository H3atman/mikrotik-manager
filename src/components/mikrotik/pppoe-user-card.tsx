...
export function PPPoEUserCard({ user, credentials, onUpdate, onEditExpiry, disabled }: PPPoEUserCardProps) {
  ...
  // Fetch profiles when profile select dialog opens
  useEffect(() => {
    if (showProfileSelectDialog) {
      const getProfiles = async () => {
        try {
          const profileList = await fetchPPPoEProfiles(credentials);
          // Filter out the restricted profiles from selection
          const filteredProfiles = profileList.filter(
            (p: string) => p !== 'Due_Date_512Kbps' && p !== 'DisCon_Date'
          );
          setProfiles(filteredProfiles);
          // Set default selection to first available profile
  ...
  }, [showProfileSelectDialog, credentials, selectedProfile]);
  ...
  // Check if user needs profile selection (restricted profiles)
  const needsProfileSelection = user.profile === 'Due_Date_512Kbps' || user.profile === 'DisCon_Date';
  ...
  const handlePaidClick = () => {
    if (needsProfileSelection) {
      ...
      // Directly set next month expiry
      handleSetNextMonthExpiry();
    }
  };

  const handleSetNextMonthExpiry = async (newProfile?: string) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const updatePayload: { comment: string; profile?: string } = {
        comment: newComment
      };
      
      // If a new profile is provided (from modal), include it in the update
      if (newProfile) {
        updatePayload.profile = newProfile;
        console.log('Changing profile to:', newProfile);
      }
      
      await updatePPPoEUser(credentials, user.id, updatePayload);
  ...
  const handleProfileSelectConfirm = () => {
    if (selectedProfile) {
      handleSetNextMonthExpiry(selectedProfile);
    }
  };
    ...
    <>
      ...
      <Dialog open={showProfileSelectDialog} onOpenChange={setShowProfileSelectDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Select Profile for {user.name}</DialogTitle>
            <DialogDescription>
              This user is currently on a restricted profile ({user.profile}). 
              Please select the appropriate profile to apply after marking as paid.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              The expiry date will be set to the 1st of next month.
            </p>
          </div>
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button
              onClick={handleProfileSelectConfirm}
